function [Ym LR] = survey_evaluation_malicious(Z0,X0,N,S,K,alpha0,thres,Nfr,Sfr)
...
% Ym: malicious candidate surveys
...
[SV1trues, SVs, SCs] = survey(Z0,N,S,K,100,Nfr,Sfr);
for i=1:100
Yt{i}(1,:)=SCs(i,:)/sum(SCs(i,:));
Yt{i}(2,:)=SVs(i,:)/sum(SVs(i,:));
end
for i=1:100
k=find(mnrnd(1,ones(1,K)/K)==1);
vv=zeros(1,K);vv(k)=1;
Ym{i}(1,:)=0.8*Yt{i}(1,:)+0.2*vv;
Ym{i}(2,:)=0.8*Yt{i}(2,:)+0.2*vv;
end
...
y21=0.99*Ym{j}(1,:)+0.01*(1/K)*ones(1,K);
y22=0.99*Ym{j}(2,:)+0.01*(1/K)*ones(1,K);
